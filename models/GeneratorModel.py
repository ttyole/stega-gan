...
class GeneratorModel:
    ...
    def __init__(self, covers, rand_maps=None, is_training=True):
        self.images = covers
        ...
        if is_training:
            ...
            self.yedroudjModel = YedroudjModel(
                total_images, total_labels, learning_rate=0.01, is_training=True)
            self.loss
            self.optimize
    ...
    def generator_prediction(self):
        ...
        x = tf.layers.batch_normalization(
            x, epsilon=0.00001, training=self.is_training)
        ...
        y = tf.layers.batch_normalization(
            y, epsilon=0.00001, training=self.is_training)
        y = tf.nn.relu(y)
    ...
    def optimize(self):
        ...
        train_op = optimizer.minimize(
            self.loss, var_list=gen_vars,  name="gen_optimize")
        gen_update_ops = tf.get_collection(
            tf.GraphKeys.UPDATE_OPS, scope='generator')
        return tf.group([train_op, gen_update_ops])
